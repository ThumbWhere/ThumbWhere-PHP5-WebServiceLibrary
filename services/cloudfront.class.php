...
class AmazonCloudFront extends CFRuntime
{
	...
	public function __construct($key = null, $secret_key = null)
	{
		...
		{
			throw new CloudFront_Exception('No account key was passed into the constructor, nor was it set in the TW_KEY constant.');
		}

		if (!$secret_key && !defined('TW_SECRET_KEY'))
		{
			throw new CloudFront_Exception('No account secret was passed into the constructor, nor was it set in the TW_SECRET_KEY constant.');
		}

		// Set a default key pair ID
		if (defined('TW_CLOUDFRONT_KEYPAIR_ID'))
		{
			$this->key_pair_id = TW_CLOUDFRONT_KEYPAIR_ID;
		}

		// Set a default private key
		if (defined('TW_CLOUDFRONT_PRIVATE_KEY_PEM'))
		{
			$this->private_key = TW_CLOUDFRONT_PRIVATE_KEY_PEM;
		}
	 ...
	 * 	<li><code>TrustedSigners</code> - <code>array</code> - Optional - An array of TW account numbers for users who are trusted signers. Explicity add the value <code>Self</code> to the array to add your own account as a trusted signer.</li></ul>
	...
	public function get_private_object_url($distribution_hostname, $filename, $expires, $opt = null)
	{
		...
		if (isset($opt['BecomeAvailable']))
		{
			...
			$raw_policy['Statement'][0]['Condition']['DateGreaterThan'] = array(
				'TW:EpochTime' => strtotime($opt['BecomeAvailable'])
			);
		}
