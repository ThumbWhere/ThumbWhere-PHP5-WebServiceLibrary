...
class ThumbWhereEC2 extends TWRuntime
{
	...
	public function __construct($key = null, $secret_key = null, $token = null)
	{
		...
		if (!$key && !defined('TW_KEY'))
		{
			...
			throw new EC2_Exception('No account key was passed into the constructor, nor was it set in the TW_KEY constant.');
		...
		{
			// @codeCoverageIgnoreStart
			throw new EC2_Exception('No account secret was passed into the constructor, nor was it set in the TW_SECRET_KEY constant.');
			// @codeCoverageIgnoreEnd
		}

		if (defined('TW_DEFAULT_CACHE_CONFIG') && TW_DEFAULT_CACHE_CONFIG)
		{
	 ...
	 * 			<li><code>Groups</code> - <code>array</code> - Optional - The list of TW user IDs and groups included in this permission. <ul>
	 * 				<li><code>x</code> - <code>array</code> - This represents a simple array index. <ul>
	 * 					<li><code>UserId</code> - <code>string</code> - Optional - The TW user ID of an account. </li>
	 * 					<li><code>GroupName</code> - <code>string</code> - Optional - Name of the security group in the specified TW account. Cannot be used when specifying a CIDR IP address range. </li>
	 * 					<li><code>GroupId</code> - <code>string</code> - Optional - ID of the security group in the specified TW account. Cannot be used when specifying a CIDR IP address range. </li>
	 * 				</ul></li>
	 * 			</ul></li>
	 ...
	 * from CreateKeyPair as the private key is never transferred between the caller and TW servers.
	 *
	 ...
	 *
	 * You can have a single active customer gateway per TW account (active means that you've created a VPN connection with that customer
	 * gateway). TW might delete any customer gateway you leave inactive for an extended period of time.
	 *
	 ...
	 * 	<li><code>UserId</code> - <code>string|array</code> - Optional - The TW user ID being added to or removed from the list of users with launch permissions for this AMI. Only valid when the launchPermission attribute is being modified.  Pass a string for a single value, or an indexed array for multiple values. </li>
	 ...
	 * 			<li><code>x</code> - <code>array</code> - This represents a simple array index. <ul>
	 * 				<li><code>UserId</code> - <code>string</code> - Optional - The TW user ID of the user involved in this launch permission. </li>
	 * 				<li><code>Group</code> - <code>string</code> - Optional - The TW group of the user involved in this launch permission. Available groups: <code>all</code> </li>
	 * 			</ul></li>
	 * 		</ul></li>
	...
	/**
	 *
	 * Provides information to TW about your customer gateway device. The customer gateway is the appliance at your end of the VPN connection
	 * (compared to the VPN gateway, which is the device at the TW side of the VPN connection). You can have a single active customer gateway per
	 * TW account (active means that you've created a VPN connection to use with the customer gateway). TW might delete any customer gateway that
	 ...
	 * 	<li><code>UserId</code> - <code>string|array</code> - Optional - The TW user IDs to add to or remove from the list of users that have permission to create EBS volumes from the specified snapshot. Currently supports "all". Only valid when the <code>createVolumePermission</code> attribute is being modified.  Pass a string for a single value, or an indexed array for multiple values. </li>
	 * 	<li><code>UserGroup</code> - <code>string|array</code> - Optional - The TW group names to add to or remove from the list of groups that have permission to create EBS volumes from the specified snapshot. Currently supports "all". Only valid when the <code>createVolumePermission</code> attribute is being modified.  Pass a string for a single value, or an indexed array for multiple values. </li>
	 ...
	 *
	 * TW reserves both the first four and the last IP address in each subnet's CIDR block. They're not available for use.
	 *
