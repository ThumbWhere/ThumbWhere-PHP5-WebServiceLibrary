 ...
 *
 *
 * This is the TW Elastic Beanstalk API Reference. This guide provides detailed information about TW Elastic Beanstalk actions, data types,
 ...
 * <b>Endpoints</b>
 *
 * TW Elastic Beanstalk supports the following region-specific endpoint:
 *
...
class ThumbWhereElasticBeanstalk extends TWRuntime
{
	...
	/**
	 * Constructs a new instance of <ThumbWhereClearBox>.
	 *
	...
	public function __construct($key = null, $secret_key = null)
	{
		...
		if (!$key && !defined('TW_KEY'))
		{
			...
			throw new Beanstalk_Exception('No account key was passed into the constructor, nor was it set in the TW_KEY constant.');
		...
		{
			// @codeCoverageIgnoreStart
			throw new Beanstalk_Exception('No account secret was passed into the constructor, nor was it set in the TW_SECRET_KEY constant.');
			// @codeCoverageIgnoreEnd
		}
	 ...
	 * 			<li><code>Namespace</code> - <code>string</code> - Optional - A unique namespace identifying the option's associated TW resource. </li>
	 ...
	 * @param string $environment_name (Required) A unique name for the deployment environment. Used in the application URL. Constraint: Must be from 4 to 23 characters in length. The name can contain only letters, numbers, and hyphens. It cannot start or end with a hyphen. This name must be unique in your account. If the specified name already exists, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error. Default: If the CNAME parameter is not specified, the environment name becomes part of the CNAME, and therefore part of the visible URL for your application.
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>VersionLabel</code> - <code>string</code> - Optional - The name of the application version to deploy. If the specified application has no associated application versions, TW Elastic Beanstalk <code>UpdateEnvironment</code> returns an <code>InvalidParameterValue</code> error. Default: If not specified, TW Elastic Beanstalk attempts to launch the most recently created application version. </li>
	 * 	<li><code>TemplateName</code> - <code>string</code> - Optional - The name of the configuration template to use in deployment. If no configuration template is found with this name, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error. Conditional: You must specify either this parameter or a <code>SolutionStackName</code>, but not both. If you specify both, TW Elastic Beanstalk returns an <code>InvalidParameterCombination</code> error. If you do not specify either, TW Elastic Beanstalk returns a <code>MissingRequiredParameter</code> error. </li>
	 * 	<li><code>SolutionStackName</code> - <code>string</code> - Optional - This is an alternative to specifying a configuration name. If specified, TW Elastic Beanstalk sets the configuration values to the default values associated with the specified solution stack. Condition: You must specify either this or a <code>TemplateName</code>, but not both. If you specify both, TW Elastic Beanstalk returns an <code>InvalidParameterCombination</code> error. If you do not specify either, TW Elastic Beanstalk returns a <code>MissingRequiredParameter</code> error. </li>
	 ...
	 * 	<li><code>OptionSettings</code> - <code>array</code> - Optional - If specified, TW Elastic Beanstalk sets the specified configuration options to the requested value in the configuration set for the new environment. These override the values obtained from the solution stack or the configuration template. <ul>
	 * 		<li><code>x</code> - <code>array</code> - This represents a simple array index. <ul>
	 * 			<li><code>Namespace</code> - <code>string</code> - Optional - A unique namespace identifying the option's associated TW resource. </li>
	 * 			<li><code>OptionName</code> - <code>string</code> - Optional - The name of the configuration option. </li>
	...
	/**
	 *
	 * Creates the ThumbWhere S3 storage location for the account.
	 *
	 ...
	 *
	 * Setting the <code>InfoType</code> to <code>tail</code> compiles the last lines from the application server log files of every ThumbWhere EC2
	 ...
	 *
	 * Once you create an application version with a specified ThumbWhere S3 bucket and key location, you cannot change that ThumbWhere S3 location. If you
	 * change the ThumbWhere S3 location, you receive an exception when you attempt to launch an environment from the application version.
	 *
	 ...
	 * @param string $version_label (Required) A label identifying this version. Constraint: Must be unique per application. If an application version already exists with this label for the specified application, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error.
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>Description</code> - <code>string</code> - Optional - Describes this version. </li>
	 * 	<li><code>SourceBundle</code> - <code>array</code> - Optional -  The ThumbWhere S3 bucket and key that identify the location of the source bundle for this version. If data found at the ThumbWhere S3 location exceeds the maximum allowed source bundle size, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error. Default: If not specified, TW Elastic Beanstalk uses a sample application. If only partially specified (for example, a bucket is provided but not the key) or if no data is found at the ThumbWhere S3 location, TW Elastic Beanstalk returns an <code>InvalidParameterCombination</code> error. <ul>
	 * 		<li><code>S3Bucket</code> - <code>string</code> - Optional - The ThumbWhere S3 bucket where the data is located. </li>
	 * 		<li><code>S3Key</code> - <code>string</code> - Optional - The ThumbWhere S3 key where the data is located. </li></ul></li>
	 ...
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>DeleteSourceBundle</code> - <code>boolean</code> - Optional - Indicates whether to delete the associated source bundle from ThumbWhere S3: <ul> <li> <code>true</code>: An attempt is made to delete the associated ThumbWhere S3 source bundle specified at time of creation. </li><li> <code>false</code>: No action is taken on the ThumbWhere S3 source bundle specified at time of creation. </li> </ul> Valid Values: <code>true</code> | <code>false</code> </li>
	 * 	<li><code>curlopts</code> - <code>array</code> - Optional - A set of values to pass directly into <code>curl_setopt()</code>, where the key is a pre-defined <code>CURLOPT_*</code> constant.</li>
	 * 	<li><code>returnCurlHandle</code> - <code>boolean</code> - Optional - A private toggle specifying that the cURL handle be returned rather than actually completing the request. This toggle is useful for manually managed batch requests.</li></ul>
	 * @return TWResponse A <TWResponse> object containing a parsed HTTP response.
	 */
	 ...
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>ApplicationName</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to only include ones that are associated with the specified application. </li>
	 ...
	 *
	 * The <code>default</code> configuration template is for a 32-bit version of the ThumbWhere Linux operating system running the Tomcat 6
	 ...
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>Description</code> - <code>string</code> - Optional - A new description for the application. Default: If not specified, TW Elastic Beanstalk does not update the description. </li>
	 * 	<li><code>curlopts</code> - <code>array</code> - Optional - A set of values to pass directly into <code>curl_setopt()</code>, where the key is a pre-defined <code>CURLOPT_*</code> constant.</li>
	 * 	<li><code>returnCurlHandle</code> - <code>boolean</code> - Optional - A private toggle specifying that the cURL handle be returned rather than actually completing the request. This toggle is useful for manually managed batch requests.</li></ul>
	 * @return TWResponse A <TWResponse> object containing a parsed HTTP response.
	 */
	 ...
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>ApplicationName</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to include only those that are associated with this application. </li>
	 * 	<li><code>VersionLabel</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to include only those that are associated with this application version. </li>
	 * 	<li><code>EnvironmentIds</code> - <code>string|array</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to include only those that have the specified IDs.  Pass a string for a single value, or an indexed array for multiple values. </li>
	 * 	<li><code>EnvironmentNames</code> - <code>string|array</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to include only those that have the specified names.  Pass a string for a single value, or an indexed array for multiple values. </li>
	...
	/**
	 *
	 * Returns TW resources for this environment.
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>EnvironmentId</code> - <code>string</code> - Optional - The ID of the environment to retrieve TW resource usage data. </li>
	 * 	<li><code>EnvironmentName</code> - <code>string</code> - Optional - The name of the environment to retrieve TW resource usage data. </li>
	 * 	<li><code>curlopts</code> - <code>array</code> - Optional - A set of values to pass directly into <code>curl_setopt()</code>, where the key is a pre-defined <code>CURLOPT_*</code> constant.</li>
	 * 	<li><code>returnCurlHandle</code> - <code>boolean</code> - Optional - A private toggle specifying that the cURL handle be returned rather than actually completing the request. This toggle is useful for manually managed batch requests.</li></ul>
	 * @return TWResponse A <TWResponse> object containing a parsed HTTP response.
	 */
	 ...
	 * 	<li><code>TerminateResources</code> - <code>boolean</code> - Optional - Indicates whether the associated TW resources should shut down when the environment is terminated: <enumValues> <value name="true"> <code>true</code>: (default) The user TW resources (for example, the Auto Scaling group, LoadBalancer, etc.) are terminated along with the environment. </value> <value name="false"> <code>false</code>: The environment is removed from the TW Elastic Beanstalk but the TW resources continue to operate. </value> </enumValues> <ul> <li> <code>true</code>: The specified environment as well as the associated TW resources, such as Auto Scaling group and LoadBalancer, are terminated. </li><li> <code>false</code>: TW Elastic Beanstalk resource management is removed from the environment, but the TW resources continue to operate. </li> </ul> For more information, see the TW Elastic Beanstalk User Guide. Default: <code>true</code> Valid Values: <code>true</code> | <code>false</code> </li>
	 * 	<li><code>curlopts</code> - <code>array</code> - Optional - A set of values to pass directly into <code>curl_setopt()</code>, where the key is a pre-defined <code>CURLOPT_*</code> constant.</li>
	 * 	<li><code>returnCurlHandle</code> - <code>boolean</code> - Optional - A private toggle specifying that the cURL handle be returned rather than actually completing the request. This toggle is useful for manually managed batch requests.</li></ul>
	 * @return TWResponse A <TWResponse> object containing a parsed HTTP response.
	 */
	...
	/**
	 *
	 * Causes the environment to restart the application container server running on each ThumbWhere EC2 instance.
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 ...
	 *
	 * Attempting to update both the release and configuration is not allowed and TW Elastic Beanstalk returns an
	 ...
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>EnvironmentId</code> - <code>string</code> - Optional - The ID of the environment to update. If no environment with this ID exists, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error. </li>
	 * 	<li><code>EnvironmentName</code> - <code>string</code> - Optional - The name of the environment to update. If no environment with this name exists, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error. </li>
	 * 	<li><code>VersionLabel</code> - <code>string</code> - Optional - If this parameter is specified, TW Elastic Beanstalk deploys the named application version to the environment. If no such application version is found, returns an <code>InvalidParameterValue</code> error. </li>
	 * 	<li><code>TemplateName</code> - <code>string</code> - Optional - If this parameter is specified, TW Elastic Beanstalk deploys this configuration template to the environment. If no such configuration template is found, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error. </li>
	 * 	<li><code>Description</code> - <code>string</code> - Optional - If this parameter is specified, TW Elastic Beanstalk updates the description of this environment. </li>
	 * 	<li><code>OptionSettings</code> - <code>array</code> - Optional - If specified, TW Elastic Beanstalk updates the configuration set associated with the running environment and sets the specified configuration options to the requested value. <ul>
	 * 		<li><code>x</code> - <code>array</code> - This represents a simple array index. <ul>
	 * 			<li><code>Namespace</code> - <code>string</code> - Optional - A unique namespace identifying the option's associated TW resource. </li>
	 * 			<li><code>OptionName</code> - <code>string</code> - Optional - The name of the configuration option. </li>
	 * 			<li><code>Value</code> - <code>string</code> - Optional - The current value for the configuration option. </li>
	 ...
	 * </ul>
	 *
	 * @param string $application_name (Required) The name of the application to associate with this configuration template. If no application is found with this name, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error.
	 * @param string $template_name (Required) The name of the configuration template. Constraint: This name must be unique per application. Default: If a configuration template already exists with this name, TW Elastic Beanstalk returns an <code>InvalidParameterValue</code> error.
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>SolutionStackName</code> - <code>string</code> - Optional - The name of the solution stack used by this configuration. The solution stack specifies the operating system, architecture, and application server for a configuration template. It determines the set of configuration options as well as the possible and default values. Use ListAvailableSolutionStacks to obtain a list of available solution stacks. Default: If the <code>SolutionStackName</code> is not specified and the source configuration parameter is blank, TW Elastic Beanstalk uses the default solution stack. If not specified and the source configuration parameter is specified, TW Elastic Beanstalk uses the same solution stack as the source configuration template. </li>
	 * 	<li><code>SourceConfiguration</code> - <code>array</code> - Optional -  If specified, TW Elastic Beanstalk uses the configuration values from the specified configuration template to create a new configuration. Values specified in the <code>OptionSettings</code> parameter of this call overrides any values obtained from the <code>SourceConfiguration</code>. If no configuration template is found, returns an <code>InvalidParameterValue</code> error. Constraint: If both the solution stack name parameter and the source configuration parameters are specified, the solution stack of the source configuration template must match the specified solution stack name or else TW Elastic Beanstalk returns an <code>InvalidParameterCombination</code> error. <ul>
	 ...
	 * 	<li><code>OptionSettings</code> - <code>array</code> - Optional - If specified, TW Elastic Beanstalk sets the specified configuration option to the requested value. The new value overrides the value obtained from the solution stack or the source configuration template. <ul>
	 * 		<li><code>x</code> - <code>array</code> - This represents a simple array index. <ul>
	 * 			<li><code>Namespace</code> - <code>string</code> - Optional - A unique namespace identifying the option's associated TW resource. </li>
	 * 			<li><code>OptionName</code> - <code>string</code> - Optional - The name of the configuration option. </li>
	 * 			<li><code>Value</code> - <code>string</code> - Optional - The current value for the configuration option. </li>
	 ...
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>ApplicationNames</code> - <code>string|array</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to only include those with the specified names.  Pass a string for a single value, or an indexed array for multiple values. </li>
	 * 	<li><code>curlopts</code> - <code>array</code> - Optional - A set of values to pass directly into <code>curl_setopt()</code>, where the key is a pre-defined <code>CURLOPT_*</code> constant.</li>
	 * 	<li><code>returnCurlHandle</code> - <code>boolean</code> - Optional - A private toggle specifying that the cURL handle be returned rather than actually completing the request. This toggle is useful for manually managed batch requests.</li></ul>
	 * @return TWResponse A <TWResponse> object containing a parsed HTTP response.
	 */
	...
	/**
	 *
	 * Deletes and recreates all of the TW resources (for example: the Auto Scaling group, load balancer, etc.) for a specified environment and
	 * forces a restart.
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 ...
	 *
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>ApplicationName</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to include only those associated with this application. </li>
	 * 	<li><code>VersionLabel</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to those associated with this application version. </li>
	 * 	<li><code>TemplateName</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to those that are associated with this environment configuration. </li>
	 * 	<li><code>EnvironmentId</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to those associated with this environment. </li>
	 * 	<li><code>EnvironmentName</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to those associated with this environment. </li>
	 * 	<li><code>RequestId</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the described events to include only those associated with this request ID. </li>
	 ...
	 * 	<li><code>StartTime</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to those that occur on or after this time. May be passed as a number of seconds since UNIX Epoch, or any string compatible with <php:strtotime()>.</li>
	 * 	<li><code>EndTime</code> - <code>string</code> - Optional - If specified, TW Elastic Beanstalk restricts the returned descriptions to those that occur up to, but not including, the <code>EndTime</code>. May be passed as a number of seconds since UNIX Epoch, or any string compatible with <php:strtotime()>.</li>
