...
class ThumbWhereS3 extends TWRuntime
{
	...
	public function __construct($key = null, $secret_key = null, $token = null)
	{
		...
		if (!$key && !defined('TW_KEY'))
		{
			...
			throw new S3_Exception('No account key was passed into the constructor, nor was it set in the TW_KEY constant.');
		...
		{
			// @codeCoverageIgnoreStart
			throw new S3_Exception('No account secret was passed into the constructor, nor was it set in the TW_SECRET_KEY constant.');
			// @codeCoverageIgnoreEnd
		}
	...
	public function authenticate($bucket, $opt = null, $location = null, $redirects = 0, $nothing = null)
	{
		...
		$request->add_header('Authorization', 'TW ' . $this->key . ':' . $signature);

		// If we're generating a URL, return certain data to the calling method.
		if (isset($opt['preauth']) && (integer) $opt['preauth'] > 0)
		{
			return $this->request_url . $conjunction . 'TWAccessKeyId=' . $this->key . '&Expires=' . $headers['Expires'] . '&Signature=' . rawurlencode($signature);
		}
		...
		$headers['x-tw-request-url'] = $this->request_url;
		$headers['x-tw-redirects'] = $redirects;
		$headers['x-tw-stringtosign'] = $string_to_sign;
		$headers['x-tw-requestheaders'] = $request->request_headers;

		// Did we have a request body?
		if (isset($opt['body']))
		{
			$headers['x-tw-requestbody'] = $opt['body'];
		}
	...
	public function set_bucket_acl($bucket, $acl = self::ACL_PRIVATE, $opt = null)
	{
		...
		// @codeCoverageIgnoreStart
		if (!defined('TW_CANONICAL_ID') || !defined('TW_CANONICAL_NAME'))
		{
			// Fetch the data live.
			$canonical = $this->get_canonical_user_id();
			define('TW_CANONICAL_ID', $canonical['id']);
			define('TW_CANONICAL_NAME', $canonical['display_name']);
		}
		// @codeCoverageIgnoreEnd

		if (is_array($acl))
		{
			$opt['body'] = $this->generate_access_policy(TW_CANONICAL_ID, TW_CANONICAL_NAME, $acl);
		}
		else
		{
			$opt['body'] = '';
	 ...
	 *
	 * @param string $canonical_id (Required) The canonical ID for the bucket owner. Use the `TW_CANONICAL_ID` constant or the `id` return value from <get_canonical_user_id()>.
