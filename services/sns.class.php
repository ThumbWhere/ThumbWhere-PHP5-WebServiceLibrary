...
class ThumbWhereSNS extends TWRuntime
{
	...
	public function __construct($key = null, $secret_key = null, $token = null)
	{
		...
		if (!$key && !defined('TW_KEY'))
		{
			...
			throw new SNS_Exception('No account key was passed into the constructor, nor was it set in the TW_KEY constant.');
		...
		{
			// @codeCoverageIgnoreStart
			throw new SNS_Exception('No account secret was passed into the constructor, nor was it set in the TW_SECRET_KEY constant.');
			// @codeCoverageIgnoreEnd
		}
	...
	/**
	 *
	 * The AddPermission action adds a statement to a topic's access control policy, granting access for the specified TW accounts to the
	 * specified actions.
	 *
	 * @param string $topic_arn (Required) The ARN of the topic whose access control policy you wish to modify.
	 * @param string $label (Required) A unique identifier for the new policy statement.
	 * @param string|array $account_id (Required) The TW account IDs of the users (principals) who will be given access to the specified actions. The users must have TW accounts, but do not need to be signed up for this service.  Pass a string for a single value, or an indexed array for multiple values.
	 ...
	 * or the its topic's owner can unsubscribe, and an TW signature is required. If the Unsubscribe call does not require authentication and the
