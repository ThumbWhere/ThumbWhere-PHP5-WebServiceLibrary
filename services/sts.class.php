 ...
 *
 *
 * This is the TW Security Token Service (STS) API Reference. STS is a web service that enables you to request temporary, limited-privilege
 ...
 * links to related content in <a href="http://docs.amazonwebservices.com/IAM/latest/UserGuide/">Using TW Identity and Access Management</a>.
 *
 * For more detailed information about using this service, go to <a
 * href="http://docs.amazonwebservices.com/IAM/latest/UserGuide/TokenBasedAuth.html">Granting Temporary Access to Your TW Resources</a>, in
 * <i>Using TW Identity and Access Management</i>.
 *
 * For specific information about setting up signatures and authorization through the API, go to <a
 * href="http://docs.amazonwebservices.com/IAM/latest/UserGuide/IAM_UsingQueryAPI.html">Making Query Requests</a> in <i>Using TW Identity and
 * Access Management</i>.
 *
...
class AmazonSTS extends CFRuntime
{
	...
	public function __construct($key = null, $secret_key = null)
	{
		...
		if (!$key && !defined('TW_KEY'))
		{
			...
			throw new STS_Exception('No account key was passed into the constructor, nor was it set in the TW_KEY constant.');
		...
		{
			// @codeCoverageIgnoreStart
			throw new STS_Exception('No account secret was passed into the constructor, nor was it set in the TW_SECRET_KEY constant.');
			// @codeCoverageIgnoreEnd
		}
	...
	/**
	 *
	 * The GetSessionToken action returns a set of temporary credentials for an TW account or IAM User. The credentials consist of an Access Key
	 ...
	 * users can be between one and 36 hours, with a default of 12 hours. The session duration for TW account owners is restricted to one hour.
	 *
	 ...
	 * IAM Users</a> in <i>Using TW Identity and Access Management</i>.
	 *
	 ...
	 * <i>Using TW Identity and Access Management</i>. For information about using GetFederationToken to create temporary credentials, see <a
	 ...
	 * Federated Users</a> in <i>Using TW Identity and Access Management</i>.
	 *
	 * @param string $name (Required) The name of the federated user associated with the credentials. For information about limitations on user names, go to Limitations on IAM Entities in <i>Using TW Identity and Access Management</i>.
	 * @param array $opt (Optional) An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>Policy</code> - <code>string</code> - Optional - A policy specifying the permissions to associate with the credentials. The caller can delegate their own permissions by specifying a policy, and both policies will be checked when a service call is made. For more information about how permissions work in the context of temporary credentials, see Controlling Permissions in Temporary Credentials in <i>Using TW Identity and Access Management</i>. </li>
